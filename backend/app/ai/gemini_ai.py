...
class GeminiAI(AISupervisor):
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not api_key or api_key.startswith("your_"):
            raise ValueError(
                "Valid GOOGLE_API_KEY required for GeminiAI implementation. "
                "Set in .env file or provide via settings."
            )
        genai.configure(api_key=api_key)
        self.model = "gemini-1.5-pro-latest"  # Latest available model
...
"""
    ...
    def evaluate(self, input_data: AIInput) -> AIOutput:
        """Evaluate trading opportunity using Gemini AI."""
        try:
